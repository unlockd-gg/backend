...
class UserEventUpdateFirebaseHandler(BaseHandler):
    def post(self):
        logging.info("[TASK] UserEventUpdateFirebaseHandler")

        ## this is a user key
        key_id = self.request.get('key_id')

        entity = UsersController().get_by_key_id(int(key_id))
        if not entity:
        ...
        recent_events = eventFeedController.get_recent_by_userKeyId(entity.key.id())

        if len(recent_events) > 0:
            ## build out the json
            event_array = []
            for recent_event in recent_events:
                event_array.append(recent_event.to_json_for_user())
        ...
        URL = "https://ue4topia.firebaseio.com/users-public/%s/eventfeed.json" % entity.key.id()
