...
class playerDataUpdateHandler(BaseHandler):
    def post(self):
        """
        update a player
        ...
        # get the game player
        if 'playerKeyId' in jsonobject:
            game_player = gamePlayerController.get_by_key_id(jsonobject['playerKeyId'])
            if not game_player:
            ...
            logging.info('did not find playerKeyId in request')
            return self.render_json_response(
                authorization = True,
                error= "The game playerKeyId was not found.",
                request_successful = False
                )
        ...
        ## update the player record with all of the incoming data

        if 'rank' in jsonobject:
            game_player.rank = jsonobject['rank']

        if 'score' in jsonobject:
            game_player.score = jsonobject['score']

        if 'experience' in jsonobject:
            game_player.experience = jsonobject['experience']

        if 'level' in jsonobject:
            game_player.level = jsonobject['level']
